...
@auth_bp.route('/forgot-password', methods=['POST'])
def forgot_password():
    """Request a password-reset OTP via email. Requires CAPTCHA. Never reveals whether
    an account exists for the given email (always returns the same generic message)."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No input data provided'}), 400
...
@auth_bp.route('/reset-password', methods=['POST'])
def reset_password():
    """Confirm the emailed OTP and set a new password."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No input data provided'}), 400
...
@auth_bp.route('/forgot-password', methods=['POST'])
def forgot_password():
    """Request a password-reset OTP by email. Always returns a generic message
    so we never reveal whether an email address has an account."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No input data provided'}), 400
...
@auth_bp.route('/reset-password', methods=['POST'])
def reset_password():
    """Confirm the emailed reset OTP and set a new password."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No input data provided'}), 400
...
@auth_bp.route('/forgot-password', methods=['POST'])
def forgot_password():
    """Request a password-reset OTP by email. Requires CAPTCHA. Never reveals
    whether an email is registered -- always returns the same generic message."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No input data provided'}), 400
    ...
    mail_result = send_password_reset_email(user, otp_code)

    response = {'message': generic_message}
    if not mail_result.get('sent'):
        response['dev_note'] = 'Email not configured yet -- reset code was logged on the server console.'
