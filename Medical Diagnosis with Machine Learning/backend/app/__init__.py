...
def create_app(config_name=None):
    ...
    # Instantiate (not just reference) the config class so that any
    # validation logic in __init__ (e.g. ProductionConfig's required
    # SECRET_KEY / JWT_SECRET_KEY checks) actually runs. Flask's
    # from_object() only reads class/instance attributes and never
    # calls __init__ on a bare class, so passing the class directly
    # silently skips these safety checks.
